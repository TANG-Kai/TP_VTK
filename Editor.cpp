...
IsoSurfacer* Editor::extractIsoSurface(const double &isoValue){
  ...
  IsoSurfacer *isosurfacer = IsoSurfacer::New();
  
  isosurfacer->SetInput(inputMesh_);

  isosurfacer->SetValue(isoValue);
...
vtkContourGrid* Editor::extractIsoSurfaceVTK(const double &isoValue){
  ...
  vtkContourGrid *isosurfacer = vtkContourGrid::New();
  
  isosurfacer->SetInputData(this->getData());
  
  isosurfacer->SetValue(0, isoValue);
  isosurfacer->Update();
